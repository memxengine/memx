...
- **Create:** `apps/server/src/routes/queue.ts`, `apps/server/src/queue/approve.ts`, `apps/server/src/queue/policy.ts`, `apps/server/src/queue/resolve.ts`
- **Modify:** `apps/server/src/routes/index.ts` (mount queue routes), `apps/server/src/services/ingest.ts` (emit candidates instead of writing directly), `packages/shared/src/contracts.ts` (Zod schemas for candidate payloads)
- **Delete:** any direct-write code paths in current ingest (if present)

### Downstream dependents

- `apps/server/src/services/ingest.ts` (Grep: 0 references to docs-write outside) — MUST be updated to emit candidates.
- `apps/mcp/src/tools/write.ts` — the MCP `write` tool is the primary ingest caller today; rewire to `POST /candidates` (internally) instead of direct doc insert.
