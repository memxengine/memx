...
- **NOT IN NON-GOALS (revision 2026-04-25)** — streaming WAS originally non-goal but Christian flipped it: yes, ship streaming as part of v1 since it's the gængse standard and feels significantly better on long answers. All three backends support SSE-style streaming natively. See "Open Questions" below for full reasoning.
...
**All resolved 2026-04-25 by Christian. Decisions locked in below.**

1. **In-process MCP handler co-location** — ✅ **RESOLVED: lift to `packages/core/src/mcp-tools/`.** Single source of truth — both MCP subprocess (CLI backend) and in-process router (OpenRouter backend) call the same handler functions. Refactor lands as part of Phase 2 alongside `OpenRouterChatBackend`.
2. **Per-tenant credit cap via F156** — ✅ **RESOLVED: chat now burns credits, gated by canonical pricing-table in F156.** F156 was updated 2026-04-25 to (a) flip chat from "background, free" to "credit-burning operation", (b) introduce `apps/server/src/data/chat-pricing.yaml` as editable canonical with per-model `credits_per_turn_estimate` + per-tier `daily_chat_turns_max` cap + alert thresholds, (c) default `default_model = google/gemini-2.5-flash` across ALL tiers (Hobby through Business) — premium models are bevidste opgraderinger via F152, not silent drift. Anti-burn disciplin: ~0.1 credits/turn on Flash means a Hobby tenant gets ~1000 chats per monthly 100-credit pulje. `runChat()` queries `tenant_credits.balance` + the daily cap before fanning out; structured "out of credits" error returned otherwise.
3. **Claude API direct backend** — ✅ **RESOLVED: ship `ClaudeAPIBackend` in v1.** Lower latency than OpenRouter routing through to Anthropic, and "når vi endelig betaler for claude API skal vi også høste maksimalt og det er på hastighed" (Christian). Three backends in v1 instead of two: `ClaudeCLIBackend` (dev/Max-Plan), `ClaudeAPIBackend` (prod premium), `OpenRouterBackend` (prod default + multi-model fallback chain).
4. **Streaming responses** — ✅ **RESOLVED: yes, ship streaming in v1 — it's the gængse standard.** All three backends support it natively: `claude -p --output-format stream-json`, Anthropic API `stream: true` (SSE), OpenRouter `stream: true` (SSE). The chat route's response shape changes from `application/json` to `text/event-stream` SSE; admin chat panel consumes via EventSource. Adds 1-2 days to Phase 2 for the protocol switch + UI consumer; worth it because non-streaming chat feels slow on long answers (Christian: "den er vel den gængse standard").
5. **Per-conversation backend memory** — ✅ **RESOLVED: per-turn re-resolution from chain.** If turn 3 falls back to Gemini, turn 4 still tries Claude first. Revisit if fall-flapping wastes obvious latency in practice.
6. **Tool-call max-turns budget** — ✅ **RESOLVED: bump default `CHAT_MAX_TURNS` from 5 to 8.** 5 is too low for compound tool-use chains (search → read → search → read → answer = 4 turns; one extra for self-correction = 5; no headroom). 8 gives room for genuine multi-step research while still bounding wall-clock via the 60s timeout. Same value caps both backends. Env-tunable for users who want tighter control.
