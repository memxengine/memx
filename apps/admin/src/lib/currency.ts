...
function formatDkk(cents: number, fx: FxRate): string {
  ...
  if (dkk < 0.01) {
    ...
    // doesn't collapse to "0.00 kr".
    return `${prefix}${dkk.toFixed(4)} kr`;
  }
  if (dkk < 100) {
    // 1 øre–99 kr → 2 decimals so 3¢ reads "≈0.21 kr" not "0 kr"
    return `${prefix}${dkk.toFixed(2)} kr`;
  }
  // 100+ kr → whole kroner
  return `${prefix}${Math.round(dkk)} kr`;
}
